...
describe("EmDash guide compliance: manifest contract", () => {
  it("declares every guide-required capability used by this plugin", () => {
    ...
    const plugin = createPlugin() as Record<string, unknown>;
  ...
  it("does not rely on legacy allowedHosts manifest fields", () => {
    ...
    expect(plugin.allowedHosts).toBeUndefined();
  });
  ...
  it("uses explicit hostnames only and never wildcard hosts", () => {
    ...
    const descriptorHostnames = ((descriptor.network as { allowedHostnames?: unknown } | undefined)?.allowedHostnames ?? []) as unknown[];
    const pluginHostnames = ((plugin.network as { allowedHostnames?: unknown } | undefined)?.allowedHostnames ?? []) as unknown[];
...
describe("EmDash guide compliance: source-level contract", () => {
  ...
  it("does not use undocumented cron lifecycle bucket as a top-level manifest hook", () => {
    const source = readIndexSource();
    expect(source).not.toMatch(/\bcron\s*:\s*\{/);
  });

  it("exposes guide-documented lifecycle handlers instead", () => {
    const source = readIndexSource();

    expect(source).toMatch(/\bonInstall\b|\bonActivate\b|\bonDeactivate\b|\bonRequest\b/);
  });
});
